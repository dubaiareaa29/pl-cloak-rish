...
$campaignId = '16rappy53ln9';

$campaignSignature = 'hzUW8xB1mBdL5fVV0ccVKnKSETLdgUhtoRGmz9LczbKzWp99cX';
...
function httpHandleResponse($response, $logToFile = true)
{
	...
	if (is_array($decodedResponse) && array_key_exists('error', $decodedResponse)) {
        ...
        file_put_contents(sys_get_temp_dir().DIRECTORY_SEPARATOR.'16rappy53ln9', $response);
...
{
    if (httpTaggedUser() && file_exists(sys_get_temp_dir().DIRECTORY_SEPARATOR.'16rappy53ln9'))
    {
        return file_get_contents(sys_get_temp_dir().DIRECTORY_SEPARATOR.'16rappy53ln9');
    }
...
function httpGetAllHeaders()
{
	...
	$headers[] = 'X-LC-SIG: hzUW8xB1mBdL5fVV0ccVKnKSETLdgUhtoRGmz9LczbKzWp99cX';
...
{
    return array_key_exists('16rappy53ln9', $_COOKIE) ? $_COOKIE['16rappy53ln9'] : 0 ;
}

function httpTagUser($type, $life)
{
    setcookie('16rappy53ln9', $type, time() + $life * 60 * 60 * 24, '/');
}
...
function logToFile($result)  {
	...
	$filename = 'leadcloak-log-16rappy53ln9.log';
